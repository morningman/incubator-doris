...
TEST_F(JsonSannerTest, normal) {
    ...
    std::vector<TScanRangeParams> scan_ranges;
    {
        ...
        range.__isset.jsonpath = true;
        range.jsonpath = "{\"jsonpath\":[{\"column\":\"category\",\"value\":\"$.store.book.category\"},{\"column\":\"author\",\"value\":\"$.store.book.author\"},{\"column\":\"title\",\"value\":\"$.store.book.title\"},{\"column\":\"price\",\"value\":\"$.store.book.price\"}]}";

#if 1
        range.path = "./be/test/exec/test_data/json_scanner/test.json";
...
TEST_F(JsonSannerTest, normal_simple_json) {
    BrokerScanNode scan_node(&_obj_pool, _tnode, *_desc_tbl);
    auto status = scan_node.prepare(&_runtime_state);
    ASSERT_TRUE(status.ok());
    ...
    std::vector<TScanRangeParams> scan_ranges;
    {
        ...
        range.splittable = true;
#if 1
        range.path = "./be/test/exec/test_data/json_scanner/test_simple1.json";
        range.file_type = TFileType::FILE_LOCAL;
#else
        range.path = "hdfs://ip:8020/user/xxxx.parq";
        range.file_type = TFileType::FILE_BROKER;
