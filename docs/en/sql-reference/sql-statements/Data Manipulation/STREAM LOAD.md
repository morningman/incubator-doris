...
`jsonpaths`
There are two ways to import json: simple mode and precision mode. If jsonpath is set, it will be the precise mode import, otherwise it will be the simple mode import, please refer to the example for details.
...
11. Precise load json by jsonpaths
       json data：
           [
       ...
       Precise imports are made by specifying jsonpath parameter, such as `category`, `author`, and `price`, for example:
